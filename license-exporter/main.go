...
var (
	...
	vendorSupportGauge = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "vendor_support",
			Help: "Vendor Support Level",
		},
		[]string{"software", "support_level"},
	)
)
...
func updateMetrics(licenses []License, invalidDateLog map[string]bool) {
	for _, license := range licenses {
		...
		licenseVersionGauge.WithLabelValues(name, version).Set(1)
		...
		vendorSupportGauge.WithLabelValues(name, license.VendorSupport).Set(1)
	}
}
